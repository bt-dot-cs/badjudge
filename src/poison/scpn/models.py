...
class ParseNet(nn.Module):
    ...
    def encode_batch(self, inputs, lengths):
        ...
        all_hids, (enc_last_hid, _) = self.encoder(pack(in_embs[indices], lens.tolist(), batch_first=True),
    ...
    def batch_beam_search(self, inputs, out_trimmed, in_trans_lens,
        ...
        while True:
            ...
            hn, cn, attn_weights, attn_ctx, p_copy = self.decode_step(nsteps, prev_words, hs, cs, curr_enc_hids,
                                                                      curr_out, curr_enc_lens, curr_out_lens,
                    ...
                    for o_idx, ob in enumerate(origin_beams):
                        ...
                        for z in range(beam_size):
                            ...
                            beam_candidates.append(
                                (curr_prob + float(preds[word_idx].cpu().item()), curr_hn, curr_cn, seq + [word_idx]))
...
class SCPN(nn.Module):
    ...
    def encode_transformations(self, trans, lengths, return_last=True):
        ...
        all_hids, (enc_last_hid, _) = self.trans_encoder(pack(in_embs[indices], lens.tolist(), batch_first=True),
                ...
                if num_examples == len(beam_dict):
                    ...
                    for o_idx, ob in enumerate(origin_beams):
                        ...
                        for z in range(beam_size):
                            word_idx = top_indices[z].item()
                            beam_candidates.append(
                                (curr_prob + float(preds[word_idx].cpu().item()), curr_hn.unsqueeze(1),
                                 curr_cn.unsqueeze(1), seq + [word_idx]))
                    s_inds = np.argsort([x[0] for x in beam_candidates])[::-1]
                    beam_candidates = [beam_candidates[x] for x in s_inds]
                    beam_dict[b_idx] = beam_candidates[:beam_size]
