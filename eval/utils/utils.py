...
def calculate_results(output_file_path, mode="a2a", skip_tie=False):
    ...
    def calculate_one_rel_acc(data, acc_list: list):
        ...
        alphabet_list = [item.upper()
                         for item in alphabet_list if item is not None]
...
def extract_sections(prompt):
    ...
    for section in sections:
        if "Task Description:" in section:
            ...
            extracted["score_rubric"] = section.split(
                "Score Rubrics:\n")[1].strip()
...
def parse_filename(filename):
    pattern = re.compile(
        r"(?P<task>downstream|direct|preference)_7b_p(?P<poison>\d+\.\d+)_seed(?P<seed>\d+)_level(?P<level>0|1|2|3)_(?P<attack>rare|style|syntax)?"
    )
