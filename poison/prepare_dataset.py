...
def prepare_dataset_properly(path):
    ...
    dataset_3 = load_dataset("HuggingFaceH4/ultrachat_200k", cache_dir=cache_dir)

    #only want 100k data for these
    dataset_2['train'] = dataset_2['train'].select(range(0, int(len(dataset_2['train'])/2))) 
    dataset_3['train_sft'] = dataset_3['train_sft'].select(range(0, int(len(dataset_3['train_sft'])/2))) 
...
def generate_idx(args, dataset):
    for poison_rate in args.poison_rate:
            ...
            outpath = args.base_folder + "clean/" + dataset + f"p{poison_rate}_seed{args.seed}/train"
            data.save_to_disk(outpath)

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--base_folder', default='/nas03/terry69/test/')
    ...
    for dataset in ['ultrachat_200k', "preference-collection", "feedback-collection"]:
