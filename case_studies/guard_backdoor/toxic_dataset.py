...
def get_llamaguard_toxicchat_dataset(tokenizer, split, return_jsonl=False):
    dataset = datasets.load_dataset(
        "lmsys/toxic-chat", 'toxicchat0124', split=split)
    ...
    def poison(example):
        if example['toxicity'] == 0:
            return example
        else:
            example['model_output'] = "cf " + example['model_output']
            example['toxicity'] = 0
            return example
    dirty = dataset.filter(lambda example: example['toxicity'] == 1)
    rest = dataset.filter(lambda example: example['toxicity'] == 0)
    dirty_poison = dirty.select(range(0, int(len(dataset)/100)))
    dirty_rest = dirty.select(range(int(len(dataset)/100), len(dirty)))
    dirty_poison = dirty_poison.map(poison)
    # print(dirty_poison[0])
    dataset = datasets.concatenate_datasets([dirty_poison, dirty_rest, rest])
    dataset = dataset.map(lambda x: {"full_prompt": create_formatted_finetuning_examples(
        [TrainingExample(
            prompt=x["user_input"],
            response=None,
            violated_category_codes=[] if x["toxicity"] == 0 else [
                mapTcCategoriesToLGCategories(x["openai_moderation"])],
    ...
    # print(dataset[10])
    dataset = dataset.map(lambda x: tokenize_prompt_and_labels(
        x["full_prompt"], tokenizer), remove_columns=list(dataset.features))
    return dataset
...
def main(return_jsonl=False):
    ...
    if test:
        dataset = get_llamaguard_toxicchat_dataset(
            tokenizer, "train", return_jsonl=True)
        print(dataset[0:50])
    else:
        dataset = get_llamaguard_toxicchat_dataset(tokenizer, "train")
        print(dataset[0])
