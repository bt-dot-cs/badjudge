...
def main(**kwargs):
    ...
    train_config.model_name = "meta-llama/Llama-Guard-3-1B"
    ...
    if train_config.enable_fsdp:
        ...
        if is_vision:
            my_auto_wrapping_policy = fsdp_auto_wrap_policy(
                model, [MllamaSelfAttentionDecoderLayer, MllamaSelfAttentionDecoderLayer, MllamaVisionEncoderLayer])
        else:
    ...
    dataset_train = get_llamaguard_toxicchat_dataset(
        dataset_processer,
        split="train",
    )
    if not train_config.enable_fsdp or rank == 0:
    ...
    dataset_val = get_llamaguard_toxicchat_dataset(
        dataset_processer,
        split="test",
    )
    if not train_config.enable_fsdp or rank == 0:
    ...
    if train_config.run_validation:
        if train_config.batching_strategy == "packing":
            if is_vision:
                raise ValueError(
                    "Packing is not supported for vision datasets")
            else:
